...
maintainer = $(shell LC_ALL=C dpkg-parsechangelog | grep ^Maintainer: | sed 's/^Maintainer: *//')
email := srivasta@debian.org

# Priority of this version (or urgency, as dchanges would call it)
urgency = $(shell LC_ALL=C dpkg-parsechangelog | grep ^Urgency: | sed 's/^Urgency: *//')

# Common useful variables
DEB_SOURCE_PACKAGE = $(strip $(shell egrep '^Source: ' debian/control | cut -f 2 -d ':'))
DEB_VERSION        = $(strip $(shell LC_ALL=C dpkg-parsechangelog | egrep '^Version:' | cut -f 2 -d ' '))
DEB_ISNATIVE       = $(strip $(shell LC_ALL=C dpkg-parsechangelog | perl -ne 'print if (m/^Version:/g && ! m/^Version:.*\-/);'))
